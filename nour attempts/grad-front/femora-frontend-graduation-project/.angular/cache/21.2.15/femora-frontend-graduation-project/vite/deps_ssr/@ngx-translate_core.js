...
var TranslateStore = class _TranslateStore {
  ...
  _translationChange$ = new import_rxjs2.Subject();
...
var makeObservable = (value) => {
  return (0, import_rxjs2.isObservable)(value) ? value : (0, import_rxjs2.of)(value);
};
var TranslateService = class _TranslateService {
  ...
  _onLangChange = new import_rxjs2.Subject();
  _onFallbackLangChange = new import_rxjs2.Subject();
  _currentLang = signal(
    null,
    ...ngDevMode ? [{
  ...
  chainTranslationChange$() {
    ...
    return streams.length === 1 ? streams[0] : (0, import_rxjs2.merge)(...streams);
  }
  /**
  ...
  get onLangChange() {
    ...
    return this.parent ? this.parent.onLangChange : import_rxjs2.EMPTY;
  }
  /**
  ...
  get onFallbackLangChange() {
    ...
    return this.parent ? this.parent.onFallbackLangChange : import_rxjs2.EMPTY;
  }
  /**
  ...
  get onTranslationRefresh() {
    if (!this._onTranslationRefresh) {
      const refresh$ = (0, import_rxjs2.merge)(this.onTranslationChange.pipe((0, import_operators2.filter)((event) => event.lang === this.getCurrentLang() || event.lang === this.getFallbackLang())), this.onLangChange, this.onFallbackLangChange).pipe((0, import_operators2.map)(() => void 0));
      if (this.isRoot) {
        this._onTranslationRefresh = refresh$;
      } else {
        this._onTranslationRefresh = this.parent ? (0, import_rxjs2.merge)(refresh$, this.parent.onTranslationRefresh) : refresh$;
      }
    }
  ...
  setFallbackLang(lang) {
    ...
    if ((0, import_rxjs2.isObservable)(pending)) {
      pending.pipe((0, import_operators2.take)(1)).subscribe({
        next: () => {
          this._fallbackLang.set(lang);
    ...
    return (0, import_rxjs2.of)(this.store.getTranslations(lang));
  }
  /**
  ...
  loadAndCompileTranslations(lang) {
    ...
    const translations$ = this.currentLoader.getTranslation(lang).pipe(
      (0, import_operators2.map)((res) => this.compiler.compileTranslations(res, lang)),
      (0, import_rxjs2.tap)((compiled) => {
      ...
      (0, import_rxjs2.finalize)(() => this.loadingTranslations.clearIfOwner(lang, translations$)),
  ...
  getParsedResultForArray(key, interpolateParams, lang) {
    ...
    for (const k of key) {
      ...
      observables = observables || (0, import_rxjs2.isObservable)(result[k]);
    }
    if (!observables) {
      return result;
    }
    const sources = key.map((k) => makeObservable(result[k]));
    return (0, import_rxjs2.forkJoin)(sources).pipe((0, import_operators2.map)((arr) => {
      const obj = {};
  ...
  get(key, interpolateParams, lang) {
    ...
    if (pending) {
      return pending.pipe((0, import_operators2.concatMap)(() => {
  ...
  getStreamOnTranslationChange(key, interpolateParams, lang) {
    ...
    return (0, import_rxjs2.concat)((0, import_rxjs2.defer)(() => this.get(key, interpolateParams, lang)), this.onTranslationChange.pipe((0, import_operators2.switchMap)(() => {
  ...
  stream(key, interpolateParams, lang) {
    ...
    const reemit$ = lang ? (0, import_rxjs2.merge)(this.onLangChange, this.chainTranslationChange$().pipe((0, import_operators2.filter)((e) => e.lang === lang))) : this.onLangChange;
    return (0, import_rxjs2.concat)((0, import_rxjs2.defer)(() => this.get(key, interpolateParams, lang)), reemit$.pipe((0, import_operators2.switchMap)(() => {
      const res = this.getParsedResult(key, interpolateParams, lang);
      return makeObservable(res);
    })));
  }
  ...
  instant(key, interpolateParams, lang) {
    ...
    return (0, import_rxjs2.isObservable)(result) ? this.keyToObject(key) : result;
  }
