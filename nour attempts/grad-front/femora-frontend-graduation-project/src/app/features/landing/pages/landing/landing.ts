  ...
  private initScrollAnimations(): void {
    ...
    document.querySelectorAll<HTMLElement>('.section-reveal').forEach((el) => {
      gsap.fromTo(
        el,
        { opacity: 0, y: 55 },
        { opacity: 1, y: 0, duration: dur, ease, scrollTrigger: { trigger: el, start: startAt } },
      );
    });
    ...
    document.querySelectorAll<HTMLElement>('.slide-right').forEach((el) => {
      gsap.fromTo(
        el,
        { opacity: 0, x: 70 },
        { opacity: 1, x: 0, duration: dur, ease, scrollTrigger: { trigger: el, start: startAt } },
      );
    });
    ...
    // 3. Card stagger — each card pops in with delay
    document.querySelectorAll<HTMLElement>('.cards-stagger').forEach((container) => {
      ...
      gsap.fromTo(
        items,
        { opacity: 0, y: 40, scale: 0.94, rotation: -1.5 },
        {
          opacity: 1,
          y: 0,
          scale: 1,
    ...
    document.querySelectorAll<HTMLElement>('.hero-inline-title').forEach((el) => {
      gsap.fromTo(
        el,
        { opacity: 0, y: 18 },
        { opacity: 1, y: 0, duration: 0.8, ease: 'power3.out', delay: 0.1, scrollTrigger: { trigger: el, start: 'top 90%' } },
      );
    });
  }
}
